class Room < Struct.new(:encrypted_name)
  def checksum
    encrypted_name[/\[(.*)\]/, 1]
  end
...
describe Room do
  it "extracts the checksum" do
    Room.new("aaaaa-bbb-z-y-x-123[abxyz]").checksum.must_equal "abxyz"
  end

  it "extracts the sector id" do
    Room.new("aaaaa-bbb-z-y-x-123[abxyz]").sector_id.must_equal 123
  end
  ...
  it "generates checksums" do
    Room.new("aaaaa-bbb-z-y-x-123[abxyz]").generate_checksum.must_equal "abxyz"
    Room.new("a-b-c-d-e-f-g-h-987[abcde]").generate_checksum.must_equal "abcde"
    Room.new("not-a-real-room-404[oarel]").generate_checksum.must_equal "oarel"
  end

  it "detects decoy rooms" do
    Room.new("aaaaa-bbb-z-y-x-123[abxyz]").valid_checksum?.must_equal true
    Room.new("a-b-c-d-e-f-g-h-987[abcde]").valid_checksum?.must_equal true
