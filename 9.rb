...
class Decompressor
  ...
  def expand_marker
    times    = marker_repeat
    sequence = scan_count(marker_count)
    sequence * times
  end

  def marker_repeat
    Integer(scanner.matched[/\(\d+x(\d+)\)/, 1])
  end
  ...
  def scan_count(count)
    count.times.map { scanner.getch }.join
  end

  def skip_count(count)
    count.times do scanner.getch end
  end
end

class DecompressorV2 < Decompressor
  attr_reader :magnitudes

  def initialize(*)
    super
    @magnitudes = [1] * data.chars.length
  end

  def decompressed_length
    until scanner.eos?
      if scan_marker
        increment_magnitudes
        reset_marker_counts
      else
        skip_count(1)
      end
      ...
      # Debugging output
      # puts "#{magnitudes.zip(data.chars).map(&:join)}, rest: #{scanner.rest}"
    end
  ...
  def reset_marker_counts
    ((scanner.pos - scanner.matched.length)...scanner.pos).each do |index|
      magnitudes[index] = 0
    end
  end

  def increment_magnitudes
    (scanner.pos...(scanner.pos + marker_count)).each do |index|
      magnitudes[index] *= marker_repeat
    end
  end
end
